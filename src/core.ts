...
function scriptTemplate(version: string, options: any) {
  try {
    ...
    function showNotification() {
      ...
      if (options.customNotificationHTML) {
        div.innerHTML = options.customNotificationHTML;
      } else {
      ...
      fetch(versionUrl)
        ...
        .then(data => {
          if (data.version && data.version !== initialVersion && !hasShownUpdate) {
            ...
            const event = new CustomEvent('plugin-web-update-notice', { 
              detail: { version: data.version, options: options } 
            });
